...
from .json_utils import Dump as jsonDump
...
def ExportGroundtruthToCOCO(image_ids,
    ...
    if output_path:
        ...
        with open(output_path, 'w') as fid:
            jsonDump(groundtruth_dict, fid, float_digits=4, indent=2)
...
def ExportDetectionsToCOCO(image_ids,
    ...
    if output_path:
        # with tf.gfile.GFile(output_path, 'w') as fid:
        with open(output_path, 'w') as fid:
            jsonDump(detections_export_list, fid, float_digits=4, indent=2)
...
def ExportSegmentsToCOCO(image_ids,
    ...
    if output_path:
        # with tf.gfile.GFile(output_path, 'w') as fid:
        with open(output_path, 'w') as fid:
            jsonDump(segment_export_list, fid, float_digits=4, indent=2)
...
def ExportKeypointsToCOCO(image_ids,
    ...
    if output_path:
        # with tf.gfile.GFile(output_path, 'w') as fid:
        with open(output_path, 'w') as fid:
            jsonDump(keypoints_export_list, fid, float_digits=4, indent=2)
